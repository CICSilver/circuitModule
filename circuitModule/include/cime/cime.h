...
#define CIME_ERR(MSG) _CIME_WRITE(CIME_ERR_STREAM, CIME_ANSI_RED, "[ERROR] ", MSG, CIME_ANSI_RESET)
    ...
    public:
        ...
        void commit()
        {
            if (!m_committed)
            {
                ...
                {
                    // 处理自增字段
                    for (auto it = m_block->getAutoIncrementMap().begin(); it != m_block->getAutoIncrementMap().end(); ++it)
                    {
                        // 自增字段值为最后一条记录同字段值加1
                        int col = it->second;
                        ...
                        else
                        {
                            ...
                            char buf[32];
                            sprintf(buf, "%d", (std::stoi(lastVal) + 1));
                            m_r.values[col] = buf;
                        }
                    }
                }
        ...
        {
            if (index < 0 || index >= m_r.values.size())
            {
    ...
    public:
        ...
        {
            auto it = m_block->m_attributes.find(key);
    ...
    public:
        ...
        {
            if (row < 0 || row >= m_block->m_records.size())
            {
